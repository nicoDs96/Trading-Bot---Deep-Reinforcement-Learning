...
def hypertune():
    ...
    while i < N_TEST:
        ...
        # SharpeDQN
        cr_sharpe_dqn_test, _ = dqn_agent.test(
            sharpe_test_env, model_name="sr_reward_dqn_model", path=models_path
        )
        ...
        cr_sharpe_ddqn_test, _ = double_dqn_agent.test(
            sharpe_test_env, model_name="sr_reward_double_dqn_model", path=models_path
        )
        sharpe_ddqn_return.append(sharpe_test_env.cumulative_return)
        sharpe_test_env.reset()

        # Sharpe Dueling Double DQN
        cr_sharpe_dueling_ddqn_test, _ = dueling_double_dqn_agent.test(
            sharpe_test_env, model_name="sr_reward_double_ddqn_model", path=models_path
        )
